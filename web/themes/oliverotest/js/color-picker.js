/**
 * @file
 * Provides UI/UX progressive enhancements on oliverotest's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.oliverotest.colorSchemes;
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-oliverotest-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-oliverotest-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes oliverotest theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-oliverotest-custom-color',
      textInput.getAttribute('data-oliverotest-custom-color'),
    );
  ...
  /**
   * oliverotest Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.oliverotestColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'oliverotest-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'oliverotest-color-picker',
        '[data-drupal-selector="oliverotest-color-picker"] input[type="text"]',
      );
