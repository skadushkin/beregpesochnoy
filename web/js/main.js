...
function sendLeadToBitrix(name, email, phone, message, source) {
    ...
    var comments = 'Страница: ' + location.href;
    if (sourceLabel) {
      comments += '\nФорма: ' + sourceLabel;
    }
...
// 		}
    ...
    jQuery('.popup-btn').click(function (e) {
        ...
        var form = jQuery(this).closest('.popup-body'); // или другой контейнер, если это не <form>
        ...
        if(form.find('textarea').val()) {
            var mess = form.find('textarea').val().trim();
        } else {
            var mess = '';
        }
...
function resetTapMenuLevels() {
    ...
    const stack = menuContainer.querySelector('[data-tap-menu-stack]');
    if (!stack) return;
...
function openTapMenuSub(panelId) {
...
if (menuContainer) {
    menuContainer.addEventListener('click', function (event) {
        ...
        if (openBtn && menuContainer.contains(openBtn)) {
            ...
            openTapMenuSub(openBtn.getAttribute('data-tap-menu-open'));
            return;
        }
...
document.querySelectorAll('.faq-bot-item').forEach(item => {
    item.addEventListener('click', function() {
        // Найти блок ответа и иконки внутри текущего элемента
        const answer = this.querySelector('.faq-answer');
        const plusIcon = this.querySelector('.faq-right-plus');
        const minusIcon = this.querySelector('.faq-right-minus');

        // Проверить состояние блока ответа и переключить его видимость
        if (answer.style.display === 'block') {
            answer.style.display = 'none';
            plusIcon.style.display = 'block';
            minusIcon.style.display = 'none';
        } else {
            // Сначала скрываем все ответы и показываем плюс
            document.querySelectorAll('.faq-answer').forEach(answer => {
                answer.style.display = 'none';
            });
            document.querySelectorAll('.faq-right-plus').forEach(icon => {
                icon.style.display = 'block';
            });
            document.querySelectorAll('.faq-right-minus').forEach(icon => {
                icon.style.display = 'none';
            });

            // Показать текущий ответ и переключить на минус
